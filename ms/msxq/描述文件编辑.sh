#!/bin/sh
perl -p -i -e "s/插件名字/CCModules「控制中心增强」/g"  cydia副本 sileo副本.json
perl -p -i -e "s/插件的描述文字/控制中心快捷开关/重启/注销/安全模式/等等/g"  cydia副本 sileo副本.json
#支持系统
perl -p -i -e "s/IOS:11-12/IOS:11-12/g"  cydia副本 sileo副本.json
perl -p -i -e "s/1.0.0/1.4-2/g"  cydia副本 sileo副本.json
#更新时间
...
mv cydia副本 safeccmodules
mv sileo副本.json safeccmodules.json
