#!/bin/sh
perl -p -i -e "s/插件名字/Cylinder「翻页特效」/g"  cydia副本 sileo副本.json
perl -p -i -e "s/插件的描述文字/桌面的翻页特效./g"  cydia副本 sileo副本.json
#支持系统
perl -p -i -e "s/IOS:11-12/IOS:11-12/g"  cydia副本 sileo副本.json
perl -p -i -e "s/1.0.0/ 1.0.7~beta1/g"  cydia副本 sileo副本.json
#更新时间
